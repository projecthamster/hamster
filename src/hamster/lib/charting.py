...
class Chart(graphics.Scene):
    ...
    def __init__(
        self, max_bar_width=20, legend_width=70, value_format="%.2f", interactive=True
    ):
    ...
    def on_mouse_over(self, scene, bar):
        if bar.key not in self.selected_keys:
            bar.fill = (
                "#999"  # self.get_style().base[gtk.StateType.PRELIGHT].to_string()
            )
    ...
    def plot(self, keys, data):
        ...
        for key, value in zip(keys, data):
            ...
            label = graphics.Label(
                stuff.escape_pango(key), size=8, alignment=pango.Alignment.RIGHT
            )
    ...
    def on_enter_frame(self, scene, context):
        ...
        for i, (label, bar) in enumerate(zip(self.labels, self.bars)):
            bar_width = min(
                round((self.plot_area.height - y) / (len(self.bars) - i)),
                self.max_width,
            )
            bar.y = y
            ...
            if bar.key in self.selected_keys:
                bar.fill = (
                    "#aaa"  # self.get_style().bg[gtk.StateType.SELECTED].to_string()
                )
    ...
    def on_enter_frame(self, scene, context):
        ...
        for i in range(start_hour + 60, end_hour, pace):
            ...
            self.layout.set_markup(
                dt.time(minutes / 60, minutes % 60).strftime(
            ...
            context.move_to(
                self.graph_x + x - label_w / 2, last_position[0] + last_position[1] + 4
            )
            g.set_color(label_color)
            context.show_layout(self.layout)
            ...
            g.line_to(
                round(self.graph_x + x) + 0.5, last_position[0] + last_position[1]
            )

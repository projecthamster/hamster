...
def from_dbus_fact(dbus_fact):
    ...
    return Fact(activity=dbus_fact[4],
                start_time=dt.datetime.fromtimestamp(dbus_fact[1], tz=dt.timezone.utc).replace(tzinfo=None),
                end_time=dt.datetime.fromtimestamp(dbus_fact[2], tz=dt.timezone.utc).replace(tzinfo=None) if dbus_fact[2] else None,

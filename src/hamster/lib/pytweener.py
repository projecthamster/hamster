...
class Tweener(object):
    ...
    def add_tween(
        ...
        **kwargs
        ...
        tw = Tween(
            obj, duration, delay, easing, on_complete, on_update, round, **kwargs
        )
    ...
    def remove_tween(self, tween):
        ...
        if (
            tween.target in self.current_tweens
            and tween in self.current_tweens[tween.target]
        ):
...
class Tween(object):
    ...
    def __init__(
        self, obj, duration, delay, easing, on_complete, on_update, round, **kwargs
    ):
...
class Tweenable(object):
    ...
    def __init__(self, start_value, target_value):
        ...
        def color_update(fraction):
            val = [
                max(min(self.start_value[i] + self.change[i] * fraction, 255), 0)
                for i in range(3)
            ]
        ...
        if isinstance(start_value, int) or isinstance(start_value, float):
            ...
            if isinstance(start_value, dt.datetime) or isinstance(start_value, dt.date):
            ...
            elif isinstance(start_value, str) and (
                self.hex_color_normal.match(start_value)
                or self.hex_color_short.match(start_value)
            ):
                ...
                self.change = [
                    target - start
                    for start, target in zip(self.start_value, target_value)
                ]
...
class Easing(object):
    ...
    def _elastic_in(t, springiness=0, wave_length=0):
        ...
        return -(
            springiness
            * math.pow(2, 10 * t)
            * math.sin((t * t - s) * (2 * math.pi) / wave_length)
        )

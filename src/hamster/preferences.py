...
class CategoryStore(gtk.ListStore):
    ...
    def load(self):
        ...
        self.unsorted_category = self.append(
            [-1, _("Unsorted")]
        )  # all activities without category
...
class PreferencesEditor(Controller):
    ...
    def __init__(self):
        ...
        self.category_tree.enable_model_drag_dest(self.TARGETS, gdk.DragAction.MOVE)
    ...
    def load_config(self, *args):
        ...
        self.tags = [
            tag["name"] for tag in runtime.storage.get_tags(only_autocomplete=True)
        ]
    ...
    def on_category_list_drag_motion(self, treeview, drag_context, x, y, eventtime):
        ...
        if (
            ...
            and drop_position != gtk.TREE_VIEW_DROP_BEFORE
        ):
    ...
    def on_category_drop(self, treeview, context, x, y, selection, info, etime):
        model = self.category_tree.get_model()
        data = selection.data
    ...
    def on_category_list_button_released(self, tree, event):
        if event.button == 1 and tree.get_path_at_pos(int(event.x), int(event.y)):
            ...
            if (
                ...
                and self._get_selected_category() != -1
    ...
    def on_category_add_clicked(self, button):
        ...
        new_category = self.category_store.insert_before(
            self.category_store.unsorted_category, [-2, _("New category")]
        )
